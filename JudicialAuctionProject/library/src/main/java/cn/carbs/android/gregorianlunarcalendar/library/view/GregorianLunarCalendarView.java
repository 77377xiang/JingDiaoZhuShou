...
//    private static final int YEAR_SPAN = YEAR_STOP - YEAR_START + 1;
    ...
    private  int  year_stop= YEAR_STOP;

    private  int  month_start_gregorian = MONTH_START_GREGORIAN;
    private  int  month_stop_gregorian = MONTH_STOP_GREGORIAN;
   ...
   // private static final int MONTH_SPAN_GREGORIAN = month_stop_gregorian - month_start_gregorian + 1;
    ...
    private  int day_start = DAY_START;
    private  int day_stop = DAY_STOP;
    ...
    public void setYear_stpop(int year_stpop) {
        this.year_stop = year_stpop;
    }

    private  int getMONTH_SPAN_GREGORIAN(){
        return  month_stop_gregorian - month_start_gregorian + 1;
    }

    private int getYEAR_SPAN(){
        return year_stop - year_start + 1;
    }

    public void setmonth_start_gregorian(int month_start_gregorian) {
        this.month_start_gregorian = month_start_gregorian;
    }

    public void setmonth_stop_gregorian(int month_stop_gregorian) {
        this.month_stop_gregorian = month_stop_gregorian;
    }
    ...
    public void setDay_stop(int day_stop) {
        this.day_stop = day_stop;
    }
    ...
    private void setDisplayData(boolean isGregorian){
        ...
        if(isGregorian){
            if(mDisplayYearsGregorian == null){
                mDisplayYearsGregorian = new String[getYEAR_SPAN()];
                for(int i = 0; i < getYEAR_SPAN(); i++){
                    mDisplayYearsGregorian[i] = String.valueOf(year_start + i);
                }
            }
            if(mDisplayMonthsGregorian == null){
                mDisplayMonthsGregorian = new String[getMONTH_SPAN_GREGORIAN()];
                for(int i = 0; i < getMONTH_SPAN_GREGORIAN(); i++){
            ...
            if(mDisplayYearsLunar == null){
                mDisplayYearsLunar = new String[getYEAR_SPAN()];
                for(int i = 0; i < getYEAR_SPAN(); i++){
                    mDisplayYearsLunar[i] = Util.getLunarNameOfYear(i + year_start);
                }
            }
            if(mDisplayMonthsLunar == null){
                mDisplayMonthsLunar = new String[getMONTH_SPAN_GREGORIAN()];
                for(int i = 0; i < getMONTH_SPAN_GREGORIAN(); i++){
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            if(day_stop != dayStop){
                if(day_stop>dayStop){
                    Log.e(getClass().getSimpleName()," error  day_stop>dayStop ");
                }else{
                    dayStop= day_stop;
                }
            }
